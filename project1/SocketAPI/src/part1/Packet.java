...
public class Packet {
    ...
    public Packet(Header header, byte[] payload) throws IOException {
        this.header = header;
        this.payload = payload;
        this.header.setPayloadLen(this.payload.length);
    }

    public Packet(int psecret, short step, short id, byte[] payload) throws IOException {
    ...
    public void payloadBuild(short num) {
        byte[] b = ByteBuffer.allocate(4).putShort(num).array();
        payloadBuild(b);
    }

    public int getIntFromNetworkBytes(byte[] bytes, int startPos) {
        int value = ((bytes[startPos] & 0xFF) << 24) |
    ...
    public short getShortFromNetworkBytes(byte[] bytes, int startPos) {
        short value = (short) (((bytes[startPos] & 0xFF) << 8) |
                (bytes[startPos + 1] & 0xFF));
        return value;
    }
