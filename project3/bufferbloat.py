...
import os
import math
import helper # calculate avg and stdev
...
class BBTopo(Topo):
    ...
    def build(self, n=2):
        ...
        # Link h1 - s0
        linkopts = dict()
        linkopts['bw'] = args.bw_host
        linkopts['use_htb'] = True
        self.addLink(host1, switch, **linkopts)
        
        # Link h2 - s0
        linkopts = dict()
        linkopts['bw'] = args.bw_net
        linkopts['delay'] = args.delay
...
def bufferbloat():
    ...
    # loop below useful.
    transfer_times = measure_transfer_time(net, 3)
    ...
    #std_dev_time = (sum((xi - avg_time) ** 2 for xi in transfer_times) / len(transfer_times)) ** 0.5
    avg = helper.avg(transfer_times)
    stdev = helper.stdev(transfer_times)

    print("Average transfer time: ", avg)
    print("Stan Dev of transfer time: ", stdev)
...
def measure_transfer_time(net, n_times=3):
    h1 = net.get('h1')
    h2 = net.get('h2')

    # Depending on webserver setup, might need to adjust the URL.
    curl_cmd = "curl -o index.html -s -w %%{time_total} %s/http/index.html" % (h1.IP())
    ...
    for i in range(n_times):
        ...
        times = float(curl_proc.stdout.read())
        transfer_times.append(times)
